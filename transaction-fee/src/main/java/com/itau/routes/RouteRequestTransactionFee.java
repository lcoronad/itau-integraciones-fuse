...
@Component
public class RouteRequestTransactionFee extends RouteBuilder{

	private Logger logger = LoggerFactory.getLogger(RouteRequestTransactionFee.class);
	...
	public void configure() throws Exception {
		...
		.choice()				
			.when(PredicateBuilder.and(exchangeProperty("status").isEqualTo("000"), exchangeProperty("severity").isEqualTo("Info")))
				...
				.log("${exchangeProperty.transactionFee}")
				.log("${exchangeProperty.responseStatus}")
				.log("${exchangeProperty.responseTnrInfoList}")
				.bean(ResponseHandler.class)
