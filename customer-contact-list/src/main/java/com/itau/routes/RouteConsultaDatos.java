...
public class RouteConsultaDatos extends RouteBuilder{
	...
	public void configure() throws Exception {
		...
		onException(MyException.class)
			...
			.setHeader(Constant.MESSAGE, simple("${exception.message}"))
		...
		onException(CamelException.class)
 			.handled(true)
 			.log(LoggingLevel.ERROR, logger, "Proceso: ${exchangeProperty.procesoId} | Mensaje: Encontro una exception general fuera de la ruta: ${exception.message}")
		...
		from(Constant.ROUTE_CONSULTA_DATOS).routeId("ROUTE_GET_DATA").streamCaching("true")
		    ...
		    .onException(CamelException.class)
		 		.handled(true)
		 		.log(LoggingLevel.ERROR, logger, "Proceso: ${exchangeProperty.procesoId} | Mensaje: Encontro una exception general dentro de la ruta: ${exception.message}")
		 		.bean(ResponseHandler.class)
		 		.marshal().json(JsonLibrary.Jackson)
		 		.log(LoggingLevel.ERROR, logger, "Proceso: ${exchangeProperty.procesoId} | Mensaje: Filanizo \n ${body}")
				.setHeader(Exchange.CONTENT_TYPE, constant(MediaType.APPLICATION_JSON_UTF8))
		...
		.setProperty(Constant.STATUS).jsonpath("$.Body.getCustomerContactListRs.*.Status.statusCode")
		.setProperty(Constant.SEVERITY).jsonpath("$.Body.getCustomerContactListRs.*.Status.severity")
		
		.log(LoggingLevel.DEBUG, logger, "Status: ${exchangeProperty.status} | Severity: ${exchangeProperty.severity}")
				
		.choice()				
				.when(PredicateBuilder.and(exchangeProperty(Constant.STATUS).isEqualTo("000"), exchangeProperty(Constant.SEVERITY).isEqualTo("Info")))
					...
					.setHeader(Exchange.CONTENT_TYPE, constant(MediaType.APPLICATION_JSON_UTF8))				
				.endChoice()	
				.when(PredicateBuilder.and(exchangeProperty(Constant.STATUS).isEqualTo("000"), exchangeProperty(Constant.SEVERITY).isEqualTo("Warning")))
				...
				.when(PredicateBuilder.or(exchangeProperty(Constant.STATUS).convertToString().isEqualTo("120")))
					...
					.inOnly(Constant.ROUTE_EXCEPTION_STATUS_ERROR_BUS)
				.endChoice()	
				.when(PredicateBuilder.or(exchangeProperty(Constant.STATUS).convertToString().isEqualTo("150")))
