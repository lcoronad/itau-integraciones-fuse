...
public class TransformationRoute extends ConfigurationRoute {
	...
	public void configure() throws Exception {
		...
		from("direct:transformationRoute").routeId("modcustomer_transformation").streamCaching()
			.setProperty("procesoId", simple("${exchangeId}")).id("firstStep")
			.process(e -> {
			...
			.to("http4://SOAPService?throwExceptionOnFailure=false").id("consumeItauService")
		...
		from("direct:finalStep").routeId("ROUTE_FINAL")
			.log("Endprocess ${body}")
		.end();
