	...
	public void configure() throws Exception {
		...
		.choice()				
			.when(PredicateBuilder.and(exchangeProperty("status").isEqualTo("000"), exchangeProperty("severity").isEqualTo("Info")))
				...
				.setProperty(Constants.STATUS_CODE).jsonpath("$.Body.doDebitAccountRs.HeaderResponse.Status.statusCode")
	            .setProperty(Constants.SERVER_STATUS_CODE).jsonpath("$.Body.doDebitAccountRs.HeaderResponse.Status.serverStatusCode")
	            .setProperty(Constants.SEVERITY).jsonpath("$.Body.doDebitAccountRs.HeaderResponse.Status.severity")
	            .setProperty(Constants.STATUS_DESC).jsonpath("$.Body.doDebitAccountRs.HeaderResponse.Status.statusDesc")
				.bean(ResponseHandler.class)
        ...
        from(Constants.ROUTE_ERROR_120_150).routeId("ROUTE_ERROR_120_150").streamCaching()
            ...
            .setProperty(Constants.SEVERITY).jsonpath("$.Body.doDebitAccountRs.HeaderResponse.Status.severity")
            .setProperty(Constants.STATUS_DESC).jsonpath("$.Body.doDebitAccountRs.HeaderResponse.Status.statusDesc")
            .setProperty(Constants.ADDITIONAL_STATUS).jsonpath("$.Body.doDebitAccountRs.*.*.AdditionalStatus")
