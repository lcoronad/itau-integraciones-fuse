		...
		from(Constants.ROUTE_VALIDATOR_STATUS).routeId("VALIDATOR_DATA").streamCaching()
			...
			.choice()				
				.when(PredicateBuilder.and(exchangeProperty("status").isEqualTo("000"), exchangeProperty("severity").isEqualTo("Info")))
					...
					.setProperty(Constants.STATUS_CODE).jsonpath("$.Body.doCreditAccountRevRs.HeaderResponse.Status.statusCode")
		            .setProperty(Constants.SERVER_STATUS_CODE).jsonpath("$.Body.doCreditAccountRevRs.HeaderResponse.Status.serverStatusCode")
		            .setProperty(Constants.SEVERITY).jsonpath("$.Body.doCreditAccountRevRs.HeaderResponse.Status.severity")
		            .setProperty(Constants.STATUS_DESC).jsonpath("$.Body.doCreditAccountRevRs.HeaderResponse.Status.statusDesc")
					.bean(ResponseHandler.class)
			...
			from(Constants.ROUTE_EXCEPTION_STATUS_ERROR_BUS).routeId("EXCEPTION-STATUS-ERROR-BUS").streamCaching()
	            ...
	            .setProperty(Constants.SEVERITY).jsonpath("$.Body.doCreditAccountRevRs.HeaderResponse.Status.severity")
	            .setProperty(Constants.STATUS_DESC).jsonpath("$.Body.doCreditAccountRevRs.HeaderResponse.Status.statusDesc")
	            .setProperty(Constants.ADDITIONAL_STATUS).jsonpath("$.Body.doCreditAccountRevRs.*.*.AdditionalStatus")
				.log(LoggingLevel.DEBUG, logger, "Proceso: ${exchangeProperty.procesoId} | Mensaje: Busqueda ${exchangeProperty.responseStatus}")
				.setHeader(Exchange.CONTENT_TYPE, constant(MediaType.APPLICATION_JSON_UTF8))
